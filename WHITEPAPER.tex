...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small \textbf{Schiavinato Sharing RFC v0.2.0}}
...
% --- Title Data ---
\title{\vspace{-1cm} \textbf{\Large Schiavinato Sharing: Human-Executable Secret Sharing for BIP39 Mnemonics} \\[0.5em] \large A Pencil-and-Paper Arithmetic Scheme for Multi-Chain Inheritance and Disaster Recovery}
\author{\textbf{Renato Schiavinato Lopez} \\ \textit{GRIFORTIS} \\ \url{https://github.com/GRIFORTIS}}
\date{\today \\ \small \textit{Status: Request for Comments (RFC) - v0.2.0} \\ \small \textit{Comment Period: Through January 31, 2026}}
...
\begin{abstract}
\noindent The Schiavinato BIP39 Mnemonic Sharing Scheme (Schiavinato Sharing) is a threshold secret-sharing scheme for BIP39 mnemonics, designed explicitly for recovery with pencil and paper. It provides universal protection for multi-chain cryptocurrency portfolios across Bitcoin, Ethereum, and all BIP39-compatible blockchains using a single sharing instance.

The scheme instantiates Shamir's Secret Sharing over the prime field $GF(2053)$, operating directly on BIP39 word indices rather than on binary entropy. Each word index is sharded independently by a random polynomial, and additional checksum secrets provide robust detection of human arithmetic errors during manual recovery.

By pre-computing Lagrange coefficients for common threshold schemes, the scheme reduces recovery to a sequence of additions and multiplications modulo 2053. This design preserves the information-theoretic security guarantees of standard Shamir secret sharing while enabling verification without electronic devices.

GRIFORTIS has developed and released reference implementations under the MIT License, including a functional JavaScript/TypeScript library (v0.2.0), an auditable offline HTML tool with comprehensive test coverage, and reproducible test vectors for independent verification. These implementations enable integration and community scrutiny while maintaining security guarantees across the entire cryptocurrency ecosystem.
\end{abstract}

\section{Introduction}

\subsection{The Inheritance Problem in a Digital Age}
The widespread use of cryptocurrencies and other digital bearer assets has created a new inheritance problem. Long-term control over these assets is typically anchored to a single secret, such as a 24-word BIP39 mnemonic [7], which serves as the master seed for hierarchical deterministic (HD) wallets [6] across Bitcoin, Ethereum, and the vast majority of modern blockchain ecosystems. If this secret is lost or destroyed, the assets are unrecoverable; if it is exposed, the assets can be stolen. Traditional backup strategies (for example, a single steel plate in a safe) concentrate risk and often fail to account for multi-generational time horizons [4].
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Zero on-chain footprint}: Unlike multisig schemes that require on-chain setup and fund migration, Schiavinato operates purely on the BIP39 mnemonic. Users can retroactively protect existing wallets without moving funds, paying transaction fees, or exposing addresses and balances on the blockchain. Shares are generated offline from the mnemonic alone—no wallet interaction required.
...
Share revocation & No\textsuperscript{‡} & No & No & Yes & N/A \\
\bottomrule
\end{tabular}
...
\vspace{0.5em}
\textsuperscript{‡}\textit{Share revocation: Shamir-based schemes (Schiavinato, SLIP39, SSKR) cannot revoke individual shares after generation. If k shares are compromised, the wallet must be replaced entirely by moving funds to a new mnemonic. Multisig wallets can rotate individual keys without moving funds.}
\end{table*}

\subsection{Comparison with Manual-First Recovery Schemes}
...
\subsubsection{Codex32}
Codex32 [9] is a pioneering implementation of manual Shamir Secret Sharing for Bitcoin seeds, formalized as BIP-0093. It represents a significant advancement in human-computable cryptography for cryptocurrency custody.

\paragraph{Strengths:}
\begin{itemize}[noitemsep]
    \item \textbf{Strong error correction}: Employs BCH (Bose-Chaudhuri-Hocquenghem) codes to detect and correct errors during manual operations, providing mathematical guarantees against transcription mistakes.
    \item \textbf{Fully air-gapped}: All operations—generation, splitting, and recovery—can be performed without electronic devices using paper computation wheels (volvelles) and lookup tables.
    \item \textbf{Rigorous specification}: Formalized as a Bitcoin Improvement Proposal with detailed operational procedures.
\end{itemize}

\paragraph{Weaknesses:}
\begin{itemize}[noitemsep]
    \item \textbf{High complexity}: Requires mastery of volvelles (paper computation wheels) and multi-step lookup procedures, presenting a steep learning curve.
    \item \textbf{Time investment}: Manual operations are significantly more time-consuming than arithmetic-based approaches.
    \item \textbf{Custom encoding}: Uses bech32-style strings rather than standard BIP39 word lists, requiring conversion steps and limiting direct wallet compatibility.
\end{itemize}
...
\subsubsection{SeedXOR}
SeedXOR [10] takes a fundamentally different approach, using XOR operations to split BIP39 mnemonics into multiple parts.

\paragraph{Strengths:}
\begin{itemize}[noitemsep]
    \item \textbf{Simplicity}: XOR operations are conceptually straightforward and can be performed with basic tools or simple software.
    \item \textbf{Plausible deniability}: Each share appears as a valid BIP39 mnemonic, allowing users to maintain decoy wallets.
    \item \textbf{Standard format}: Operates directly on BIP39 mnemonics without custom encoding.
\end{itemize}

\paragraph{Weaknesses:}
\begin{itemize}[noitemsep]
    \item \textbf{Limited threshold flexibility}: Primarily an $n$-of-$n$ scheme where all shares are required for recovery. While a 2-of-3 Hamming backup variant exists [11], it is not widely adopted and adds significant complexity.
    \item \textbf{Weak error detection}: No robust checksums during manual XOR operations; a single bit error propagates silently through the entire recovery process, only becoming apparent when the final mnemonic fails validation.
    \item \textbf{No information-theoretic security}: Unlike Shamir-based schemes, XOR splitting does not provide threshold security—possession of $n-1$ shares still reveals no information, but the scheme offers no flexibility in threshold selection.
\end{itemize}

\subsubsection{Comparative Analysis}
Table \ref{tab:manual_comparison} compares the three manual-first approaches across key dimensions.

\begin{table*}[t]
\centering
\caption{Comparison of Manual-First Recovery Schemes}
...
\paragraph{Positioning Schiavinato Sharing:}
Schiavinato Sharing is designed with a \textbf{pencil-and-paper-first philosophy}, similar to Codex32, but makes different trade-offs in the design space. Where Codex32 optimizes for maximum error correction at the cost of operational complexity, Schiavinato prioritizes \textbf{arithmetic simplicity} (modular addition and multiplication) and \textbf{standard BIP39 compatibility}, ensuring recovered mnemonics work immediately with any existing wallet.

Critically, while maintaining full manual recoverability as the foundational design principle, Schiavinato intentionally provides \textbf{robust computational implementations}—including JavaScript/TypeScript libraries (v0.2.0), an offline HTML tool with UR QR code generation [14], and printable worksheets with embedded checksums. This dual approach serves users who want \textbf{convenience when electronics are trusted}, with manual recovery remaining available as a fallback option for disaster scenarios or when electronic tools are unavailable or untrusted.

For a broader comparison including electronic-only schemes (SLIP39, SSKR, Multisig), see Table \ref{tab:comparison} in Section 1.3.
...
\[ p = 2053, \]
the smallest prime greater than the BIP39 [7] wordlist size of 2048.
...
\begin{itemize}[noitemsep]
    ...
    \item 1 \textbf{global checksum} over all 24 words, defined as
...
This procedure is applied to each of the 33 secrets. For any fixed share index $x$ (for example, $x = 3$), the 33 resulting values (24 word shares, 8 row-checksum shares, and 1 global checksum share) together constitute \textbf{one} cryptographic share of the wallet.
...
\paragraph{Expected Recovery Time:}
Manual recovery time varies by user experience and scheme complexity. For a typical 24-word mnemonic with 2-of-3 threshold:
\begin{itemize}[noitemsep]
    \item \textbf{Calculator-based recovery}: 45--90 minutes for users comfortable with basic arithmetic
    \item \textbf{Lookup-strip method}: 60--120 minutes (includes strip lookup time)
    \item \textbf{Electronic recovery (HTML tool)}: Less than 5 minutes with QR code scanning
\end{itemize}
First-time users should expect to spend additional time familiarizing themselves with the worksheets and coefficient tables. Practice recovery on test mnemonics before attempting production recovery.
...
\paragraph{2. Global checksum (Shamir-shared):} In addition to the row-level checks, the scheme defines a global checksum
\[ G = \sum_{i=1}^{24} w_i \pmod{2053} \]
...
\paragraph{Recovery Scenarios:}
Consider two scenarios involving an authorized party attempting to recover cryptocurrency holdings:

\textbf{Scenario 1 (BIP39):} An authorized party obtains a document containing a 12- or 24-word sequence. Due to operational stress, document degradation, or confusion among multiple documents, they may enter an incorrect mnemonic sequence into a wallet application. The application validates the BIP39 checksum and displays a wallet with zero balance. \textit{Question: Did they enter the wrong mnemonic that happened to pass checksum validation, or is there truly no inheritance?}

\textbf{Scenario 2 (Schiavinato):} An authorized party gathers $k$ share documents from distributed locations (family members, attorneys, safe deposit boxes). They perform 50--100 manual arithmetic operations (multiplications and additions modulo 2053) to recover the mnemonic, verifying row checksums at each step and finally the global checksum. All checks pass. They enter the recovered mnemonic into a wallet application, which displays zero balance. \textit{Question: Did arithmetic errors escape detection, or is there truly no inheritance?}

\paragraph{BIP39 Checksum Structure:}
BIP39 mnemonics encode entropy plus a checksum derived from the SHA-256 hash of the entropy:
\begin{itemize}[noitemsep]
    \item \textbf{12-word mnemonic}: 128 bits of entropy + 4 checksum bits = 132 bits total. The last word encodes the final 4 checksum bits plus 7 entropy bits.
    \item \textbf{24-word mnemonic}: 256 bits of entropy + 8 checksum bits = 264 bits total. The last word encodes the final 8 checksum bits plus 3 entropy bits.
\end{itemize}

The checksum detects transcription errors when entering a mnemonic into a wallet, but it provides no protection against \textit{selecting the wrong mnemonic entirely}. If an authorized party enters an arbitrary sequence of BIP39 words, the probability it passes checksum validation is [12] (where $P_{\text{fp}}^{\text{BIP}}(n)$ denotes the false positive probability for BIP39 with $n$ words):
...
In practical terms, roughly \textbf{1 in 16 wrong 12-word mnemonics} and \textbf{1 in 256 wrong 24-word mnemonics} will pass BIP39 validation and produce a valid (but incorrect) wallet with zero balance.

\paragraph{Schiavinato Two-Layer Checksum Structure:}
Schiavinato Sharing employs checksums at two levels:
\begin{itemize}[noitemsep]
    \item \textbf{12-word mnemonic}: 4 rows of 3 words each, yielding 4 row checksums + 1 global checksum.
    \item \textbf{24-word mnemonic}: 8 rows of 3 words each, yielding 8 row checksums + 1 global checksum.
\end{itemize}

Each checksum is computed in $GF(2053)$ and is itself recovered via Lagrange interpolation. The row checksums are mathematically independent, while the global checksum $G = \sum_{i=1}^{n} w_i \pmod{2053}$ serves as a global consistency check that is algebraically dependent on the row checksums (since $G$ equals the sum of all row checksums). However, the global checksum provides practical value by detecting systematic errors that affect all rows proportionally and transcription errors made after recovery is complete.

For an incorrect recovery to pass all row-level validation, each row checksum must coincidentally match its recomputed value. Under the random-error model, the probability of this occurring is (where $P_{\text{fp}}^{\text{Sch}}(n)$ denotes the false positive probability for Schiavinato with $n$ words):
...
Table \ref{tab:checksum_comparison} summarizes the false-positive rates for both schemes.

\begin{table*}[t]
\centering
\caption{Checksum False Positive Rates: BIP39 vs. Schiavinato Sharing}
\label{tab:checksum_comparison}
\small
\begin{tabular}{@{}lcccc@{}}
\toprule
\textbf{Metric} & \textbf{BIP39} & \textbf{Schiavinato} & \textbf{BIP39} & \textbf{Schiavinato} \\
 & \textbf{(12-word)} & \textbf{(12-word)} & \textbf{(24-word)} & \textbf{(24-word)} \\ \midrule
Checksum structure & 4 bits & 4 row checks & 8 bits & 8 row checks \\
 & & + 1 global & & + 1 global \\
False positive rate & 1/16 & $(1/2053)^4$ & 1/256 & $(1/2053)^8$ \\
Numeric probability & 6.25\% & $5.6 \times 10^{-14}$ & 0.39\% & $1.1 \times 10^{-27}$ \\
Advantage factor & \multicolumn{2}{c}{$\sim 1.1 \times 10^{12}$ safer} & \multicolumn{2}{c}{$\sim 4.3 \times 10^{24}$ safer} \\
Practical meaning & 1 in 16 wrong & Virtually & 1 in 256 wrong & Astronomically \\
 & mnemonics validate & impossible & mnemonics validate & impossible \\
\bottomrule
\end{tabular}
\end{table*}
...
\paragraph{Practical Implications for Inheritance:}
\begin{itemize}[noitemsep]
    \item \textbf{BIP39 confusion risk}: An authorized party who obtains a document with 12 words has a 6.25\% chance (roughly 1 in 16) of entering the wrong mnemonic that still passes validation, leading to a zero-balance wallet and potential confusion about whether inheritance exists. For 24-word mnemonics, this risk drops to 0.39\% (1 in 256), but remains non-negligible in high-stakes scenarios.
    \item \textbf{Schiavinato arithmetic confidence}: Even after performing 50+ manual arithmetic operations during recovery, the probability of errors passing all row checksums is less than $10^{-13}$ for 12-word recovery and $10^{-27}$ for 24-word recovery. The additional global checksum provides a final global consistency check that catches systematic errors and post-recovery transcription mistakes. This combination provides \textit{mathematical certainty} that a validated result is correct.
    \item \textbf{Complementary strengths}: BIP39's checksum was designed to detect transcription errors when entering a known mnemonic. Schiavinato's multi-layer checksums are designed to detect arithmetic errors during a multi-step recovery process. The two serve different purposes and are not directly comparable in all contexts, but for inheritance scenarios involving manual recovery, Schiavinato provides vastly stronger error detection.
\end{itemize}
...
\begin{itemize}
    \item \textbf{Header metadata}: Wallet Identifier, Creation Date, Scheme ($k$-of-$n$), Share Number $x$, and the global checksum share value.
    \item \textbf{Primary data table}: Formatted as 8 rows × 4 columns for 24-word mnemonics (4 rows × 4 columns for 12-word mnemonics). Each row contains three word shares and one row-checksum share, displayed in large, readable type as ``Word - 1234'' format when the value corresponds to a BIP39 word index.
    \item \textbf{Machine-readable encoding}: A CBOR-encoded array of share data, formatted as a Uniform Resource (UR) QR code [14] for optional electronic recovery and long-term digital preservation.
\end{itemize}
...
\paragraph{Note: Lagrange interpolation recovers the complete polynomial.}
...
\begin{enumerate}[noitemsep]
    \item \textbf{Self-consistency check}: During manual recovery, users can verify their arithmetic by computing a known share and confirming it matches the expected value. For example, if shares $\{1, 2\}$ correctly produce share 3's values word by word and checksum by checksum, it provides strong evidence that the recovery calculations were performed correctly.
    \item \textbf{Share generation without secret exposure}: In principle, holders of $k$ shares can generate additional valid shares without explicitly computing the secret $f(0)$. While this offers no security advantage---possession of $k$ shares already permits full recovery---it demonstrates that the polynomial itself, not merely its constant term, is fully determined by $k$ points.
\end{enumerate}

It is important to emphasize that this is a fundamental mathematical property of polynomial interpolation, not a vulnerability. The security model of Shamir's Secret Sharing already assumes that any party with $k$ or more shares has complete access to the secret. The ability to derive additional shares does not weaken this guarantee; it merely reflects the fact that the underlying polynomial is uniquely determined.
...
\begin{align*}
Y \cdot \gamma &\equiv (d_3 \cdot 1000 \gamma) + (d_2 \cdot 100 \gamma) \\
&\quad + (d_1 \cdot 10 \gamma) + (d_0 \gamma) \pmod{2053}
\end{align*}
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Integrity of recovery}: Given at least $k$ honest shares, the combination of row-level checks and the global checksum should detect accidental arithmetic or transcription errors with overwhelmingly high probability.
\end{itemize}
...
\paragraph{Multi-Secret Independence:}
Because Schiavinato Sharing employs 33 independent polynomial instances (24 words + 8 row checksums + 1 global checksum), the security analysis extends beyond single-secret Shamir schemes. An adversary with $t < k$ shares for a wallet obtains:
\begin{itemize}[noitemsep]
    \item No information about any individual word index beyond its domain $\{0, \dots, 2047\}$
    \item No information about row checksums beyond their domain $\{0, \dots, 2052\}$
    \item No ability to exploit relationships between checksums and words, as each is protected by its own independent polynomial
\end{itemize}

The checksum relationships (e.g., $c_r = \sum w_{r,i} \pmod{2053}$) are public knowledge and do not constitute information leakage. With fewer than $k$ shares, conditioning on these equations does not reduce the search space for any individual secret.

\paragraph{Resistance to Partial Information Attacks:}
Unlike schemes where shares contain structured metadata or format identifiers, Schiavinato shares consist purely of field elements in $\{0, \dots, 2052\}$. An adversary who obtains $t < k$ shares cannot:
\begin{itemize}[noitemsep]
    \item Distinguish valid shares from random noise
    \item Determine which secrets correspond to high-value words vs. low-value words
    \item Extract partial entropy from share values
    \item Launch statistical attacks based on BIP39 word frequency distributions
\end{itemize}

\paragraph{Security Reduction:}
The security of Schiavinato Sharing reduces to:
\begin{enumerate}[noitemsep]
    \item The information-theoretic security of Shamir's Secret Sharing in $GF(2053)$ [1]
    \item The cryptographic security of the BIP39 mnemonic system [7]
    \item The quality of randomness used in polynomial coefficient generation
\end{enumerate}
No additional cryptographic assumptions are introduced beyond those already required by BIP39 and Shamir's original construction.
...
\begin{itemize}[noitemsep]
    ...
    \item The global checksum adds a final consistency check over all 24 recovered words, ensuring that no residual errors remain undetected.
\end{itemize}

Under the same random-error model discussed in Section \ref{sec:checksum}, the probability that an incorrect row passes its checksum test is at most $1/2053$, and the probability that such errors also preserve the (recomputed) global checksum is at most another factor of $1/2053$. With an additional independence assumption across rows this yields an overall error-escape probability on the order of $(1/2053)^9$; even without that assumption, the per-row-plus-global bound of $1/2053^2$ is already so small as to be negligible in practice.
...
\subsection{Physical Operational Security}

While side-channel attacks (timing, power analysis) are typically associated with electronic devices, manual recovery introduces physical side channels that must be managed:

\begin{itemize}[noitemsep]
    \item \textbf{Impression Attacks}: Writing arithmetic calculations on a pad of paper can leave legible indentations on the sheets underneath.
    \begin{itemize}[noitemsep]
        \item \textit{Mitigation}: Perform calculations on a hard surface (glass, wood, or a dedicated clipboard) rather than directly on a paper pad.
    \end{itemize}
    \item \textbf{Waste Management}: Calculation sheets contain intermediate values that could be used to reconstruct secrets.
    \begin{itemize}[noitemsep]
        \item \textit{Mitigation}: All scratch paper used during recovery must be destroyed immediately after use (burning or cross-cut shredding). Do not simply discard them in household trash.
    \end{itemize}
    \item \textbf{Visual Surveillance}: Manual recovery takes time and requires spreading out documents.
    \begin{itemize}[noitemsep]
        \item \textit{Mitigation}: Perform recovery in a private, windowless room or with blinds drawn. Ensure no security cameras (including laptop webcams or smart home devices) have a line of sight to the workspace.
    \end{itemize}
\end{itemize}
...
\begin{itemize}[noitemsep]
    ...
    \begin{itemize}[noitemsep]
        ...
        \item Split and recovery functions with row and global checksums
...
\begin{itemize}[noitemsep]
    ...
    \begin{itemize}[noitemsep]
        ...
        \item Global checksum share: a separate integer value for the global checksum
...
\begin{itemize}
    ...
    \item \textbf{UR QR Code Integration}: GRIFORTIS reference worksheets encode share data as CBOR arrays formatted according to the Blockchain Commons Uniform Resource (UR) specification [14]. Each worksheet displays a UR QR code alongside the primary 8×4 (or 4×4) table of human-readable word shares. This encoding serves dual purposes: (i) enabling instant electronic recovery when trusted hardware is available, bypassing manual arithmetic entirely, and (ii) providing machine-readable backup for digital preservation and verification if printed worksheets degrade over decades. The pencil-and-paper arithmetic path remains the primary recovery mechanism, ensuring the scheme's core design principle—\textit{electronics-optional at every stage}—is preserved while opportunistically leveraging technology when appropriate.
    
    \item \textbf{Extended encoding format specifications}: Future standardization efforts could define formal, implementation-independent specifications for encoding \texttt{MnemonicShare} objects in additional formats (e.g., JSON, CBOR variants, NFC tags) to facilitate interoperability across diverse implementations and hardware platforms.
    ...
    \item \textbf{Verifiable Secret Sharing extensions}: The current scheme provides information-theoretic secrecy but does not include mechanisms for participants to verify that shares were generated correctly without revealing the secret. Future work could explore adaptations of verifiable secret sharing protocols to the manual computation setting, potentially using commitment schemes that remain human-verifiable.
...
\begin{enumerate}
    ...
    \begin{itemize}[noitemsep]
        ...
        \item Transfer to an air-gapped computer (ideally booted from a temporary OS such as Tails for maximum security)
        \item Input your existing BIP39 mnemonic
        \item Generate shares and print/transcribe worksheets
        \item Verify that at least one set of $k$ shares can recover the original (test immediately)
        \item \textbf{Securely destroy temporary artifacts}:
        \begin{itemize}[noitemsep]
            \item \textbf{Digital files}: Use secure deletion tools (e.g., \texttt{shred -vfz -n 10 filename} on Linux/macOS, or \texttt{sdelete -p 10 filename} on Windows) to overwrite files with random data before deletion
            \item \textbf{Browser data}: Clear all browser cache, history, and form data; restart the browser
            \item \textbf{If using Tails OS or similar}: Simply shut down—the operating system provides amnesia by design
            \item \textbf{Printed test pages}: Physically destroy via cross-cut shredding or burning; do not discard intact
            \item \textbf{RAM}: Power down the air-gapped computer completely (not just sleep/hibernate) to clear memory
        \end{itemize}
    \end{itemize}
    
    \item \textbf{Distribute shares according to your plan}: Follow deployment patterns from Appendix E, ensuring no single location contains $k$ or more shares.
...
\begin{enumerate}
    ...
    \item \textbf{Optional: Support UR QR code recovery}: Reference worksheets include Blockchain Commons UR-encoded QR codes [14]. Scanning multiple share QR codes enables instant electronic recovery as an alternative to manual arithmetic.
...
\textbf{The Claim}: Our two-layer checksum (row-level + global checksum) detects arithmetic errors with probability $\geq 1 - (1/2053)^2$ per row under a random-error model.
...
\begin{itemize}[noitemsep]
    ...
    \item \textbf{Payment terms}: Upon qualification determination, payment via Bitcoin, Ethereum, Solana, or other mutually agreed method within 30 days
...
\begin{enumerate}[label={[\arabic*]}]
    ...
    % --- Manual Recovery Schemes ---
    \item A. Poelstra, L. O'Connor, and S. Corallo, ``BIP-0093: Codex32: Checksummed SSSS-aware BIP32 seeds,'' \textit{Bitcoin Improvement Proposals}, 2023. [Online]. Available: \url{https://github.com/bitcoin/bips/blob/master/bip-0093.mediawiki}.
    
    \item ``SeedXOR: Split your BIP39 seed for better redundancy and security,'' [Online]. Available: \url{https://seedxor.com/}.
    
    \item A. P. Goucher, ``Hamming backups: a 2-of-3 variant of SeedXOR,'' \textit{Complex Projective 4-Space}, Sept. 2021. [Online]. Available: \url{https://cp4space.hatsya.com/2021/09/10/hamming-backups-a-2-of-3-variant-of-seedxor/}.
    
    \item J. Lopp, ``How Many Bitcoin Seed Phrases Are Only One Repeated Word?,'' \textit{blog.lopp.net}, 2023. [Online]. Available: \url{https://blog.lopp.net/how-many-bitcoin-seed-phrases-are-only-one-repeated-word/}.
    
    \item J. von Neumann, ``Various techniques used in connection with random digits,'' in \textit{Monte Carlo Method}, National Bureau of Standards Applied Mathematics Series, vol. 12, pp. 36--38, 1951.
    
    \item Blockchain Commons, ``BCR-2020-005: Uniform Resources (UR): Encoding Structured Binary Data for Transport in URIs and QR Codes,'' \textit{Blockchain Commons Research}, 2020. [Online]. Available: \url{https://github.com/BlockchainCommons/Research/blob/master/papers/bcr-2020-005-ur.md}.
...
\subsection{Replicating Individual Shares for Redundancy} \label{sec:share-replication}
For users who require greater protection against physical loss without increasing scheme complexity, an alternative approach is to create \textbf{multiple copies of the same share} and distribute them to different locations.

\paragraph{The pattern:}
Instead of generating a 2-of-5 or 3-of-7 scheme with many distinct shares, consider generating a simpler 2-of-3 or 3-of-5 scheme and then creating multiple physical copies of selected shares. For example:
\begin{itemize}[noitemsep]
    \item Generate a standard 2-of-3 scheme (shares 1, 2, 3).
    \item Print or transcribe \textbf{two copies} of share 1: one stored at home, one in a bank vault.
    \item Print \textbf{two copies} of share 2: one in a second location, one with a trusted relative.
    \item Keep share 3 as a single copy in a third secure location.
\end{itemize}

\paragraph{Advantages:}
\begin{itemize}[noitemsep]
    \item \textbf{Simpler mental model}: The threshold logic remains straightforward (``any 2 of the 3 distinct shares''), even though more physical documents exist.
    \item \textbf{Localized redundancy}: Loss of a single location does not necessarily destroy that share if a copy exists elsewhere.
    \item \textbf{Easier recovery}: Heirs only need to understand and locate copies of $k$ distinct shares, not track a large combinatorial space.
    \item \textbf{No change to recovery arithmetic}: The Lagrange coefficient tables and manual recovery procedures remain identical; users simply use whichever physical copy of a share is accessible.
\end{itemize}

\paragraph{Security considerations:}
\begin{itemize}[noitemsep]
    \item \textbf{Threshold is unchanged}: Multiple copies of share 1 are still only \textbf{one share} from a cryptographic perspective. An adversary who obtains both copies has the same information as if they obtained one.
    \item \textbf{Increased attack surface}: More physical copies mean more opportunities for theft, inadvertent exposure, or unauthorized duplication. Carefully weigh the risk of loss against the risk of compromise for your specific threat model.
    \item \textbf{Clear labeling required}: All copies of a given share should be clearly and consistently labeled (e.g., ``Share 1 of 3, Copy A'' and ``Share 1 of 3, Copy B'') to avoid confusion during recovery about which shares are distinct.
\end{itemize}

\paragraph{When to use this pattern:}
This approach is particularly well-suited for:
\begin{itemize}[noitemsep]
    \item Users who are primarily concerned with \textbf{accidental loss} (fire, flood, misplacement) rather than adversarial theft.
    \item Scenarios where certain shares are more critical or harder to replace (e.g., the shares held directly by the wallet owner).
    \item Estate planning where heirs might have difficulty coordinating access to many distinct parties, but redundant copies in well-documented locations simplify the logistics.
\end{itemize}

\paragraph{Documentation is critical:}
As with all Schiavinato Sharing deployments, \textbf{clear documentation} of which shares exist, where each copy is stored, and how they should be combined is essential. Consider maintaining a non-secret inventory (without revealing share contents) that executors and heirs can consult, listing:
\begin{itemize}[noitemsep]
    \item Share identifiers (e.g., ``Share 1, Copy A'').
    \item Storage locations (e.g., ``Home safe, second drawer'').
    \item Access instructions (e.g., ``Combination in safety deposit box \#123'').
\end{itemize}

This inventory does not compromise security---it reveals no secret information---but significantly increases the likelihood of successful recovery when needed.

\subsection{Proper Share Labeling and Custody}
If acting as a custodian for multiple parties' shares (for example, a professional advisor, family member, or friend holding shares from different wallets), maintain strict separation and clear labeling to prevent confusion or inadvertent mixing of shares from different wallets.

\paragraph{Essential labeling practices:}
\begin{itemize}[noitemsep]
    \item Store each person's shares in \textbf{separate, clearly labeled envelopes or containers}.
    \item Include non-secret metadata on each share document:
    \begin{itemize}[noitemsep]
        \item \textbf{Wallet Identifier}: A user-defined label (e.g., ``Alice - Main Wallet'', ``Bob - 2025 Inheritance'', ``Retirement Fund''). This is free-form text (up to 40 characters) that should uniquely identify the wallet among all wallets being managed.
        \item \textbf{Owner/Beneficiary}: The person or entity to whom the wallet belongs.
        \item \textbf{Creation Date}: When the shares were generated.
        \item \textbf{Scheme}: The threshold configuration (e.g., ``2-of-3'', ``3-of-5'').
        \item \textbf{Share Number}: Which share this is (e.g., ``Share \#1 of 3'').
    \end{itemize}
    \item \textbf{Never} mix shares from different wallets in the same physical location or container.
    \item Keep a simple non-secret inventory (without revealing share contents) listing who owns which shares and where they are stored.
\end{itemize}

\paragraph{For manual share generation:}
When creating shares entirely by hand, write the wallet identifier clearly at the top of each blank share worksheet \textit{before} beginning any arithmetic operations. This ensures that all shares for a given wallet are consistently labeled from the outset and reduces the risk of later confusion.

\paragraph{Important security note:}
While it is theoretically possible to identify which wallet a share belongs to by using $k-1$ other shares from a suspected wallet to compute the expected values and verify them against the share in question (as discussed in the context of share replication, see Section~\ref{sec:share-replication}), \textbf{this practice is strongly discouraged}. It requires temporarily exposing $k$ shares (the $k-1$ shares used for computation plus the share being verified) to a single party, which violates the fundamental threshold security model of the scheme. Proper labeling from the outset eliminates any need for such risky verification procedures.
...
This appendix specifies one concrete way to generate uniformly random integers in $\{0, \dots, 2052\}$ using only fair six-sided dice. The procedure employs rejection sampling [13] to eliminate modulo bias, ensuring that each value in the target range has exactly equal probability. Other entropy sources are acceptable as long as they provide at least as much unpredictability.
...
Because $3 \cdot 2053 = 6159 \le 7776 < 4 \cdot 2053$, the first 6159 base-6 outcomes can be partitioned into three equal-sized blocks, each of size 2053. Restricting to this accepted region and then taking $N \bmod 2053$ yields a uniform distribution on $\{0, \dots, 2052\}$: each value appears exactly 3 times in the range $[0, 6158]$, eliminating the modulo bias that would occur if we naively reduced all 7776 outcomes modulo 2053 (which would make values 0--1722 appear 4 times while values 1723--2052 appear only 3 times). A similar rejection sampling process is used for the non-zero leading coefficient.
...
To facilitate formal verification and independent analysis, this appendix provides a concise algorithmic specification of the core primitives, stripped of user-interface concerns.

\subsection{Parameters}
\begin{itemize}[noitemsep]
    \item \textbf{Field}: $GF(p)$ with $p = 2053$.
    \item \textbf{Secret Domain}: $\mathcal{S} = \{0, \dots, 2052\}$.
    \item \textbf{Share Index Domain}: $\mathcal{X} = \{1, \dots, n\} \subset GF(p)$.
\end{itemize}

\subsection{Primitive: \texttt{Share}(s, k, n)}
\textbf{Input}: Secret $s \in \mathcal{S}$, Threshold $k \ge 1$, Count $n \ge k$. \\
\textbf{Output}: Set of shares $\{(x, y_x) \mid x \in \{1, \dots, n\}\}$.

\begin{enumerate}
    \item Set $a_0 = s$.
    \item Sample $a_1, \dots, a_{k-2}$ uniformly from $\{0, \dots, 2052\}$.
    \item Sample $a_{k-1}$ uniformly from $\{1, \dots, 2052\}$ (ensuring degree is exactly $k-1$).
    \item Define $P(z) = \sum_{j=0}^{k-1} a_j z^j \pmod{2053}$.
    \item For each $x \in \{1, \dots, n\}$, compute $y_x = P(x) \pmod{2053}$.
    \item Return $\{(x, y_x)\}_{x=1}^n$.
\end{enumerate}

\subsection{Primitive: \texttt{Recover}(Shares, k)}
\textbf{Input}: A set of $k$ distinct shares $S_{in} = \{(x_i, y_i)\}_{i=1}^k$. \\
\textbf{Output}: Secret $s \in \mathcal{S}$.

\begin{enumerate}
    \item For each $j \in \{1, \dots, k\}$, compute the Lagrange coefficient for the origin:
    \[
    ...
    \[ s = \sum_{j=1}^{k} y_j \gamma_j \pmod{2053} \]
    \item Return $s$.
\end{enumerate}

\subsection{Primitive: \texttt{ValidateRow}(Words, ChecksumShare)}
\textbf{Input}: Recovered words $w_1, w_2, w_3 \in \mathcal{S}$, Recovered checksum $c \in \mathcal{S}$. \\
\textbf{Output}: Boolean (True if valid).

\begin{enumerate}
    \item Compute expected checksum: $c_{expected} = (w_1 + w_2 + w_3) \pmod{2053}$.
    \item Return $c == c_{expected}$.
\end{enumerate}
