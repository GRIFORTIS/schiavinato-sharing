...
% --- Version ---
\newcommand{\DuraShareVersion}{v0.7.0}
...
\hypersetup{
    ...
    urlcolor=blue,
    citecolor=blue,
    pdftitle={DuraShare: BIP39-Native Threshold Backup over GF(2053) with Full Manual Fallback and Per-Share Audit},
...
\pagestyle{fancy}
\fancyhf{}
\fancyhead[L]{\small \textit{DuraShare \DuraShareVersion}}
...
% --- Title ---
\title{\textbf{DuraShare: BIP39-Native Threshold Backup over $\GF(2053)$ with Full Manual Fallback and Per-Share Audit}}
\author{
    \textbf{Renato Schiavinato Lopez} \\
    GRIFORTIS \\
    \texttt{renato@grifortis.com}
}
\date{July 2026\\\DuraShareVersion\thanks{%
    Specification, test vectors, and in-progress prototype implementations:
    \url{https://github.com/GRIFORTIS/durashare}.
...
Multi-Party Computation (MPC) wallets, multisignature schemes, and smart contract social recovery~\cite{buterin} address \emph{operational security} for active asset management. DuraShare addresses a different problem: \emph{cold storage and disaster recovery}---how to keep threshold recovery available across long time horizons without permanently binding that recovery to one software stack. The comparisons below are therefore limited to threshold backup schemes for stored secrets.
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Feature} & \textbf{DuraShare} & \textbf{Codex32} & \textbf{SeedXOR} \\ \midrule
...
\begin{tabular}{@{}lccc@{}}
\toprule
\textbf{Feature} & \textbf{DuraShare} & \textbf{SLIP39} & \textbf{SSKR} \\ \midrule
...
\paragraph{Out of scope.} The post-quantum security of the BIP39-derived wallet itself---most notably the ECDSA/EdDSA signing keys whose underlying discrete-logarithm and factoring problems are broken by Shor's algorithm~\cite{shor1994} on a sufficiently capable quantum computer---is a downstream property of the wallet ecosystem, not of this protocol. DuraShare does not introduce its own quantum-vulnerable primitives beyond those enumerated above, and it does not solve or alter the quantum status of the secret it protects.
...
R. Schiavinato Lopez,
\textit{Schiavinato Sharing v0.4.1} (historical name; now DuraShare), GRIFORTIS, 2025.
Available at \url{https://github.com/GRIFORTIS/durashare}.
